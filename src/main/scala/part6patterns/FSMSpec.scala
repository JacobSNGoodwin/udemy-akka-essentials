...
class FSMSpec extends TestKit(ActorSystem("FSMSpec"))
  ...
  def runTestSuite(props: Props) = {
    ...
    "deliver the product if I insert all of the money" in {
      val vendingMachine = system.actorOf(props)
      vendingMachine ! Initialize(Map("coke" -> 10), Map("coke" -> 3))
      vendingMachine ! RequestProduct("coke")
      expectMsg(Instruction("Please insert 3 dollars"))
    ...
    "give back change and request money for a new product" in {
      val vendingMachine = system.actorOf(props)
      vendingMachine ! Initialize(Map("coke" -> 10), Map("coke" -> 3))
      vendingMachine ! RequestProduct("coke")
      expectMsg(Instruction("Please insert 3 dollars"))
  ...
  class VendingMachine extends Actor with ActorLogging {
    ...
    def waitForMoney(
      ...
      case ReceiveMoneyTimeout =>
        ...
        context.become(operational(inventory, prices)) // no change to inventory
      case ReceiveMoney(amount) =>
        moneyTimeoutSchedule.cancel() // more time to insert next bill/coin
        val price = prices(product)
        if (money + amount >= price) {
          ...
          context.become(
            waitForMoney(
              ...
              startReceiveMoneyTimeoutSchedule, // I need a new timeout
              requester
            )
          )
        }
    ...
    def startReceiveMoneyTimeoutSchedule = context.system.scheduler.scheduleOnce(3.second){
  ...
  class VendingMachineFSM extends FSM[VendingState, VendingData] {
    ...
    when(Idle) {
      case Event(Initialize(inventory, prices), Uninitialized) =>
        goto(Operational) using Initialized(inventory, prices) // different way of writing context.become
      case _ =>
    ...
    when(WaitForMoney, stateTimeout = 3.second) {
