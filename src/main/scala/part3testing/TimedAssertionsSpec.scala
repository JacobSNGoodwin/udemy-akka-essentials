  ...
  "A worker actor" should {
    ...
    "reply with valid work at a reasonable cadence" in {
      within(1 second) {
        ...
        // idle is max time between messages
        val results: Seq[Int] = receiveWhile[Int](max = 2 seconds, idle = 100 millis, messages=10) {
    ...
    "reply to a test probe in a timely manner" in {
      within(1 second) {
        ...
        probe.expectMsg(WorkResult(42)) // timeout of 0.3seconds in akka config - separate from within setting
      }
    }
  }
