...
class SynchronousTestingSpec extends WordSpecLike with BeforeAndAfterAll {
  ...
  "A counter" should {
    "synchronously increase its counter" in {
      ...
      counter ! Inc // counter has ALREADY received the message due to TestActorRef
    ...
    "synchronously increase its counter at the call of the receive function" in {
      val counter = TestActorRef[Counter](Props[Counter])
      counter.receive(Inc)
      assert(counter.underlyingActor.count == 1) // basically the same test as above
    }
    ...
    "work on the calling thread dispatcher" in {
      ...
      probe.send(counter, Read) // every interaction with the counter happens on the probe's calling thread

      probe.expectMsg(Duration.Zero, 0) // make sure probe has already received the message 0
