...
//  child ! "hi, kid!"
//
//  parent ! StopChild("child1") // async
//
////  for (_ <- 1 to 50) child ! "are you still there?" // see if child's still there after stop
//
...
//  abruptlyTerminatedActor ! Kill // makes actor throw ActorKilledException
//  abruptlyTerminatedActor ! "you have been terminated"
  ...
  class Watcher extends Actor with ActorLogging {
    ...
    override def receive: Receive = {
      case StartChild(name) =>
        ...
        context.watch(child) // will receive Terminated message when child dies
      case Terminated(ref) =>
  ...
  Thread.sleep(500) // make sure watchedChild has been created
