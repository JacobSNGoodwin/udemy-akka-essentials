  ...
  "A checkout flow" should {
    ...
    "freak out if the payment is denied" in {
      EventFilter[RuntimeException](occurrences = 1) intercept {
        val checkoutRef = system.actorOf(Props[CheckoutActor])
        checkoutRef ! Checkout(item, "0000-0000-0000-0000")
      }
    }
  }
}
...
object InterceptingLogsSpec {
  ...
  class CheckoutActor extends Actor {
    ...
    def pendingPayment(item: String): Receive = {
      ...
      case PaymentDenied =>
        throw new RuntimeException("I can't handle this anymore!") // not a good idea, just an example
  ...
  class FulfillmentManager extends Actor with ActorLogging {
    var orderId = 43 // for simplicity, using var
    override def receive: Receive = {
